...
const Args = struct {
    ...
    pub fn init(process_init: std.process.Init) !Self {
        return try argparse.parse(process_init, Self, struct {
...
pub fn main(init: std.process.Init) !void {
    ...
    var aggregate_stats: struct {
        languages: std.StringArrayHashMapUnmanaged(u64),
        language_colors: std.StringArrayHashMapUnmanaged([]const u8),
        contributions: usize,
        name: []const u8,
    ...
    for (stats.repositories) |repository| {
        ...
        if (repository.languages) |langs| for (langs) |language| {
            ...
            if (language.color) |color| {
                try aggregate_stats.language_colors.put(allocator, language.name, color);
            }
